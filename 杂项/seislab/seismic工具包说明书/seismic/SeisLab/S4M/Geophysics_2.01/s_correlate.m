...
% Function performs crosscorrelation of two seismic data sets
...
% Last updated: April 9, 2001; bug fix
%
%               corr=s_correlate(seis1,seis2,varargin)
% INPUT
% seis1      first data set   
% seis2      second data set
%            the two seismic data sets must have the same sample interval and the difference in 
%            start time must be an integer multiple of the this sample interval.
...
if isfield(seis1,'null')
  ...
  disp([' ',ds1,' has null values; they have been replaced by zeros'])
end

if isfield(seis2,'null')
  seis2=s_rm_trace_nulls(seis2);
  disp([' ',ds2,' has null values; they have been replaced by zeros'])
end
...
elseif length(param.lags) == 2
    up=fix(param.lags{1}/seis1.step)*seis1.step;
    down=fix(param.lags{2}/seis1.step)*seis1.step;
    if up > down
...
if 0
nup=round((up-seis1.first+seis2.first)/seis1.step);
ndown=round((down-seis1.first+seis2.first)/seis1.step);
else
