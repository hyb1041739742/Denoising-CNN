...
%       'Gauss',    'Hamming',  'Hanning', 'none'
%       'Parzen',   'Papoulis', 'rect',    'triang'
%	'Hamming', 'Hanning', 'Harris', 'Papoulis'
%
...
if strcmpi(name,'Fejer')
   nh=fix(N/2);
   ind=(1:nh-1)/nh;
   h1=sin(pi*ind*(nh+1)).^2./(sin(pi*ind).^2*(nh+1));
   h=h1;
%  h= needs checking
elseif strcmp(name,'RECTANG') | strcmp(name,'RECT') | strcmp(name,'NONE'), 
   h=ones(N,1);
