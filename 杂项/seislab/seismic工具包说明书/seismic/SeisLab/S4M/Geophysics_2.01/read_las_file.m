...
% Last updated: October 19, 2004: bug fix (maximum mnemonic length was too long)
%
...
if wlog.step ~= 0
  dz=diff(wlog.curves(:,1));
  if max(dz)*0.97 > min(dz)
...
if strcmpi(buffer(1:2),'~p') 
 ...
 %           wlog.parameter_info=setfield(wlog.parameter_info,parameter,{unit,descr});
            wlog=setfield(wlog,parameter,str2num(value));
