...
% Last updated: October 30, 2003: change default for "update" from 0.5 to 0
%
...
%       'maxshift' Maximum shift (ms) allowed in either direction. It should be chosen big enough
%                  to include all possible shifts between the reference signal on the 
%                  reference trace and the corresponding signal on any other trace. See also
%                  keyword 'adapt' below.
...
%       'shifts'   Header mnemonic to use to store shifts. 
%                  General form: {'shifts',mnemonic}
...
%                  Correlation coefficients are not stored if mnemonic is an empty string
%                  Default: {'correlation','align_corr'}
%                  Default: {'method',1} normalized crosscorrelation
...
%       'option'   this keyword determines what should be done with data that are shifted 
%                  outside the time range of seisin. Options are:
%                  'extend'   extend the time range to accomodate all shifts. This is the 
...
if ntr == 1
   ...
   if ~isempty(param.shifts)
      seisout=s_header(seisout,'add',param.shifts,0,seisout.units, ...
             'Time shifts applied to traces in param');
   end
   if ~isempty(param.correlations)
      seisout=s_header(seisout,'add',param.correlations,1,'n/a', ...
...
%   idx=header_index(seisin,param.reference{1});
%   headers=seisin.headers(idx,:);
