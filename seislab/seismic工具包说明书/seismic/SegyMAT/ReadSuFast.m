...
function [seis,nt,nx]=ReadSuFast(fileid,nt,nx,byte);
  ...
  if nargin==4 | nargin==2,
    ...
    SegymatVerbose(['SU2MAT : Byteorder : ',b_order])
  else
    % USE DEFAULT BYTEORDER
    SegymatVerbose(['SU2MAT : Byteorder : DEFAULT'])
  end
  ...
  if nt==0, nt=l/nx-60; SegymatVerbose(['nt=',num2str(nt)]);end
  if nx==0, nx=l/(nt+60); SegymatVerbose(['nx=',num2str(nx)]); end,
