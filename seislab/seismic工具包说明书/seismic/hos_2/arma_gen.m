function [y,x,w,u]=arma_gen(flag,dim,N,ma,ar,snr,ma_n,ar_n,useed,n1,n2)
...
if (flag == 'e')
%  disp('Exponential');
  [u,m,v,s]=exponential(1,useed(1),N);   u=u-mean(u);
elseif (flag == 'i')
%  disp('Impulse');
  u=[1 zeros(1,N-1)];     % IMPULSE : for testing purposes...
elseif (flag == 'p')
%  disp('PAM-dim');
  u=PAM(dim,N,1,useed(1));   % sigma=1, PAM-dim
elseif (flag == 'q')
%  disp('QAM-d');
...
if (snr > 0)
  if (abs(sum(imag(u)))>0)
    randn('seed',n1);  wr=randn(1,N); 
    randn('seed',n2);  wi=randn(1,N);  w=wr+i*wi;
