...
% Function manipulates header(s) of a seismic structure
% Written by E. R.; Date March 16, 2000
% Last updated; June 30, 2001: make "units" and "description" optional if 
%                              action is 'replace'
%
...
% seismic=s_new2compact(seismic);    % Convert to compact header format
...
if ~isfield(seismic,'headers') 
  if ~strcmp(action,'add') & ~strcmp(action,'add_ne')
    ...
    if nargout == 0
       clear seismic; 
%    else
%       seismic=s_compact2new(seismic);    % Convert to new header format
    end
    return
  else
    nh=0;
