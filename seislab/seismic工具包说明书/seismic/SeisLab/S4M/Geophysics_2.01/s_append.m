...
% Unless input parameter "ttype" is {'max','min'} and input parameter "htype" is 'i'
...
% Last updated: August 20, 2003: fix bug in assignment of null value
%
%           seisout=s_append(seis1,seis2,varargin)
% INPUT 
% seis1   seismic structure
...
if ~strcmpi(seis1.units,seis2.units)
  error(['Input data sets have different depth/time/frequncy units ("',seis1.units,'" vs. "', ...
     seis2.units,'")'])
end

if seis1.step ~= seis2.step
  error([' Input data sets have different sample intervals (',num2str(seis1.step),' vs. ', ...
     num2str(seis2.step),' ',seis1.units,')'])
end

%if ~exist('ttype','var') | isempty(param.ttype)
%   ttype={'min','max'};
%end
%if ~exist('htype','var') | isempty(param.htype)
%  htype='u';
%end
...
%       Convert to compact header representation (if headers exist)
%seis1=s_new2compact(seis1);
%seis2=s_new2compact(seis2);
...
%  if seisout.first < seis1.first | seisout.first < seis2.first | ...
%     seisout.last  > seis1.last  | seisout.last  > seis2.last
%     seisout.null=null_value;
%  end
end
...
if ~isempty(find(isnan(seisout.traces)))
...
%if isfield(seisout,'null') & ~isnan(seisout.null)
%   if isfield(seis1,'null') | isfield(seis2,'null')
%      seisout.null=NaN;
%   else
%      seisout=rmfield(seisout,'null');
